...
public class ManipulationManager : MonoBehaviour
{
    ...
    private void Update()
    {
        if (attachedManipulationObject == null)
            return;
        MoveGizmo mg = attachedManipulationObject.GetComponent<MoveGizmo>();
        if(mg == null)
            return;
        
        if (Input.GetKeyDown(KeyCode.I))
            mg.SetManipulationMode(ManipulationType.Move);
        if (Input.GetKeyDown(KeyCode.O))
            mg.SetManipulationMode(ManipulationType.Rotate);
        if (Input.GetKeyDown(KeyCode.P))
            mg.SetManipulationMode(ManipulationType.Scale);
